...
class AI(commands.Cog):
    ...
    async def aigenerate(self, ctx: Context, *, prompt):
        ...
        output = await self.ai("davinci", prompt, 0.4, 500, 1, 0.5, 0)
    ...
    async def aicode(self, ctx: Context, *, prompt):
        ...
        """
        output = await self.ai("davinci-codex", prompt, 0, 64, 1.0, 0.0, 0.0, "#")
        await bot_send(ctx, output)
    ...
    async def text2speech(self, ctx: Context, *, input_text):
        ...
        if not self.text2speech_model:
            ...
            self.text2speech_model = Text2Speech.from_pretrained(
        ...
        for mssg in messages:
            await mssg.delete()
        sf.write(
            pjoin("folders", "send", "text2speech.waw"),
            wav.numpy(),
            self.text2speech_model.fs,
            "PCM_16",
        )
